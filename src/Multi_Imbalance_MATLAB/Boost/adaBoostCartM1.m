...
function [trainTime,testTime,predictResults] = adaBoostCartM1(traindata,trainlabel,testdata,Max_Iter)
tic;
Learners = {};
...
%         end
%     end
    weight(:)=weight(:)+(trainlabel(:)==predicted(:))* exp(- 1 * ( AlphaT(t)))+(trainlabel(:)~=predicted(:))* exp( AlphaT(t));
