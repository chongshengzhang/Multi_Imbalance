...
% 
% Purpose: adaBoostCartNC using a Multi-class Exponential loss function) has extended AdaBoost in both the update
...
function [trainTime,testTime,predictResults] = adaBoostCartNC(traindata,trainlabel,testdata,Max_Iter,lama)
...
%     end
% end
weight(:)=weight(:) .* ((trainlabel(:)==predicted(:)).* exp(- 1 * ( AlphaT(t))) )+ weight(:) .* ((trainlabel(:)~=predicted(:)) .* exp( AlphaT(t)*0));
...
%     end
   ...
   amb(:)=  (ResultHt(:)~=predicted(:)) .* (trainlabel(:)==predicted(:)) -1;
   amb(:)=  (ResultHt(:)~=predicted(:)) .* (trainlabel(:)~=predicted(:)) +1;
...
%     end
     ...
     weight(:)=Pweight(:) .* ((trainlabel(:)==predicted(:)).* exp(- 1 * ( AlphaT(t))) )+ Pweight(:) .* ((trainlabel(:)~=predicted(:)) .* exp( AlphaT(t)*0));
