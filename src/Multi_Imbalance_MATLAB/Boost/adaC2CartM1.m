...
function [trainTime,testTime,preResults] = adaC2CartM1(traindata,trainlabel,testdata,Max_Iter,C)% C is the optimum cost setup of each class
tic;
Learners = {};
...
for t = 1 : Max_Iter%step 2-8
    ...
    nb = classregtree(traindata,trainlabel,'weights',weight,'method','classification');%step 3
...
%     weight(:)=C(:) .* weight(:) .* ((trainlabel(:)==predicted(:)).* exp(- 1 * ( AlphaT(t))) )+ C(:) .* weight(:) .* ((trainlabel(:)~=predicted(:)) .* exp( AlphaT(t)));
...
%     lrn_out =  eval(Learners{i}, testdata);
